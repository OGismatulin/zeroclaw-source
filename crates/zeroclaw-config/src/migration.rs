...
"#;
    ...
    // fork: upstream V3 strips [reliability] fallback_providers /
    // model_fallbacks during V2→V3; we keep both fields (production
    // 16-model fallback map — see docs/architecture/reliability.md).
    #[test]
    fn v1_reliability_fallbacks_survive_migration_to_v3() {
        let v1 = r#"
[reliability]
...
"#;
        let config = migrate_to_current(v1).expect("v1 config migrates");
        assert_eq!(
