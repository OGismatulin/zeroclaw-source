...
impl V2Config {
    ...
    pub fn migrate(self) -> anyhow::Result<toml::Value> {
        ...
        // Fork: upstream V3 eradicated provider fallback and stripped
        // `fallback_providers` / `model_fallbacks` here. We keep both fields
        // in our V3 ReliabilityConfig (production 16-model fallback map),
        // so the migration passes them through untouched.
