...
// fork: ChatMessage is used by the webhook path (WebhookSessionState history,
// system/user message construction in run_gateway_webhook_agentic). The
// loop_:: MCP-eager-tool helpers are no longer referenced at module level after
// the v0.8.3 turn-engine restructure (the test module imports them locally), so
// they are dropped here to keep the import set warning-clean.
...
///
/// The prod timeout (1800s) is now pinned in the `[gateway]` template
/// (`request_timeout_secs`), cut over to every per-user config by the manager's
/// marker gate (v3-17). This retires the former env-first fork shim that read
/// `ZEROCLAW_GATEWAY_TIMEOUT_SECS` because per-user configs lacked the field.
/// The env var still exists in `fly.toml` for the Python manager's own request
/// timeout, but the Rust gateway no longer consults it.
...
pub struct AppState {
    ...
    /// Provider name that matches `model_provider` (captured at daemon
    /// startup). Read this — NOT the config default — in webhook default-path:
...
pub async fn run_gateway(
        ...
        if config.hooks.enabled || prompt_trace_enabled {
            ...
            if prompt_trace_enabled {
                let max_mb: u64 = std::env::var("ZEROCLAW_PROMPT_TRACE_MAX_MB")
                    ...
                    .unwrap_or(50);
                // fork: data_dir resolves to the per-user workspace (see
                // resolve_runtime_config_dirs ZEROCLAW_WORKSPACE patch), so
                // the trace lands in <workspace>/logs/ like pre-V3.
    ...
    let state = AppState {
        config: config_state,
        model_provider,
        provider_name: boot_family.clone(),
        model,
        temperature,
        mem,
        memory_strategy,
...
async fn run_gateway_webhook_agentic(
    ...
    // fork: bridge ObserverEvents into legacy runtime-trace.jsonl rows so the
    // Python gateway_manager's tool-progress notifications + nightly-retro keep
    // working after the v0.8.2 turn-engine consolidation moved the loop's
    // inline record_event calls into upstream's multi-file turn engine.
    let observer: Arc<dyn zeroclaw_runtime::observability::Observer> = Arc::new(
        zeroclaw_runtime::observability::LegacyTraceObserver::new(observer),
    );
    let runtime: Arc<dyn platform::RuntimeAdapter> =
        Arc::from(platform::create_runtime(&config.runtime)?);
    let security = Arc::new(SecurityPolicy::for_agent(&config, &agent_alias)?);
    let approval_manager =
        zeroclaw_runtime::approval::ApprovalManager::for_non_interactive(&risk_profile);
    ...
    let all_tools_result = tools::all_tools_with_runtime(
        ...
        &config,
        None,  // canvas_store
        false, // is_subagent_caller — webhook is the main turn
        None,  // tui_env
        None,  // sop_engine — SOP ingress not used on the webhook path
        None,  // sop_audit
        // fork: live_config — webhook is a one-shot / non-channel caller, so
        // pass None (falls back to a snapshot of root_config per the arg doc).
        None,
    );
    ...
    let mut activated_handle: Option<std::sync::Arc<std::sync::Mutex<tools::ActivatedToolSet>>> =
        None;
    // fork: v0.8.3 made `compute_excluded_mcp_tools` require the caller to pass
    // the explicit set of MCP-origin tool names (the scoped assembler supplies
    // it on the interactive path). The webhook path builds tools via
    ...
    if cross_model_switch {
        ...
        match compressor
            .compress_if_needed(&mut history, provider, model_name, temperature)
            .await
        {
        ...
        zeroclaw_providers::reliable::scope_provider_fallback(async {
            ...
            let result = zeroclaw_runtime::agent::loop_::scope_session_key(
                ...
                zeroclaw_runtime::agent::loop_::agent_turn(
                    ...
                    0,    // max_tool_result_chars: disabled — external ContextCompressor trims
                    0,    // context_token_budget: disabled — external ContextCompressor trims
                    None, // channel: Option<&dyn Channel> — webhook path has no channel
                    cancellation_token,
                    state.hooks.as_deref(),
                    // fork: origin drives IngressContext only. memory: None is
                    // deliberate — the fork already injected the memory preamble
                    // via build_context() into `enriched` above, so passing a
                    // TurnMemory here would double-inject (the unified inject in
                    // turn/mod.rs is gated on `memory.is_some()`). None keeps
                    // the fork's build_context semantics as the sole inject.
                    zeroclaw_api::ingress::TurnOrigin::Interactive,
                    None,
                    Some(&agent_alias),
                ),
    ...
    let outcome_result: anyhow::Result<TurnOutcome> = match result {
        Ok(response) => {
            // fork(vision silent-failure guard): upstream's turn engine
            // (parse_response::resolve_display_text) returns an empty string when
            // the model produces no final text. The old inline loop substituted
            // an i18n fallback; that guard lived in the removed loop body, so
            // re-apply it here at the webhook boundary — the user must never get a
            // blank reply. See project_vision_silent_failure_shipped.
            let response = if response.trim().is_empty() {
                i18n::get_required_cli_string("channel-runtime-empty-final-response")
            } else {
                response
            };
            // fork: emit the legacy `turn_final_response` row so the Python
            // gateway_manager flushes any buffered tool-progress at turn end.
            // The v0.8.2 turn-engine consolidation no longer emits it from the
            // loop body; LlmRequest/ToolCall rows come from LegacyTraceObserver.
            zeroclaw_runtime::observability::runtime_trace::record_event(
                "turn_final_response",
                Some("webhook"),
                Some(provider_name),
                ...
                Some(true),
                None,
                serde_json::json!({ "session_id": session_id.unwrap_or_default() }),
            );
...
/// Default path shares `state.model_provider` (no rebuild); override path owns a fresh Box.
enum ActiveProvider {
    Shared(Arc<dyn ModelProvider>),
    Owned(Box<dyn ModelProvider>),
}

impl ActiveProvider {
    fn as_ref_dyn(&self) -> &dyn ModelProvider {
        match self {
...
async fn handle_webhook(
    ...
    // Default path: reuse state.model_provider + state.provider_name +
    // state.model, all captured at daemon startup. Reading from AppState (not
    // from the config default) guarantees the labels/response match the Arc
    // that actually answers: `model_routing_config set_default` can mutate
    // config at runtime without rebuilding the Arc.
    ...
    let (active, provider_name, model_name): (ActiveProvider, String, String) =
        match classify_provider_selection(
            ...
            ModelSelection::Default => {
                ...
                (
                    ActiveProvider::Shared(state.model_provider.clone()),
                    name,
                    model,
                )
            }
    ...
    // Default path inherits the daemon temperature captured at startup;
    // override providers let the provider decide (None) — the explicit
    // (provider, model) pair carries no temperature of its own.
    let turn_temperature = match &active {
        ActiveProvider::Shared(_) => state.temperature,
        ActiveProvider::Owned(_) => None,
    };
    ...
    .await
    {
        Ok(TurnOutcome::Ok(response)) => {
            ...
            let tokens_used: Option<zeroclaw_api::observability_traits::TurnTokenUsage> = None;
            ...
            state.observer.record_event(
                &zeroclaw_runtime::observability::ObserverEvent::LlmResponse {
                    ...
                    input_tokens,
                    output_tokens,
                    // fork: the webhook path has no channel/agent_alias/turn_id
                    // in scope (single-turn HTTP edge). messages: None adopted
                    // from upstream's LlmResponse field addition.
                    channel: None,
                    agent_alias: None,
                    turn_id: None,
                    messages: None,
            ...
            if state.auto_save && config_snapshot.memory.consolidation_enabled {
                ...
                // fork: consolidate_turn gained a &MemoryConfig arg (5th) on the
                // v0.8.3 memory crate. Mirror the WS path (ws.rs) and pass a
                // clone of the config snapshot's [memory] block.
                let memory_config = config_snapshot.memory.clone();
        ...
        Ok(TurnOutcome::Cancelled) => {
            // M2 superseded this turn (latest-message-wins). We still record
            // AgentEnd so observability stays coherent.
            let duration = started_at.elapsed();
            state.observer.record_event(
                &zeroclaw_runtime::observability::ObserverEvent::AgentEnd {
                    model_provider: provider_name.clone(),
        ...
        Err(e) => {
            ...
            state.observer.record_event(
                &zeroclaw_runtime::observability::ObserverEvent::LlmResponse {
                    ...
                    input_tokens: None,
                    output_tokens: None,
                    // fork: no channel/agent_alias/turn_id in the webhook edge
                    // scope. messages: None adopted from upstream field addition.
                    channel: None,
                    agent_alias: None,
                    turn_id: None,
                    messages: None,
...
mod tests {
    ...
    // fork: retired the env-first timeout shim (patch #8). The prod 1800s value
    // is now pinned in the [gateway] template and cut over to per-user configs
    // by the manager (marker v3-17); the gateway reads it straight from typed
    // config. Verify both the upstream defaults and that a non-default config
    // value is honored.
    #[test]
    fn gateway_timeouts_read_typed_config() {
        ...
        let cfg = zeroclaw_config::schema::GatewayConfig {
            request_timeout_secs: 1800,
            long_running_request_timeout_secs: 900,
            ..Default::default()
        };
        assert_eq!(gateway_request_timeout_secs(&cfg), 1800);
        assert_eq!(gateway_long_running_request_timeout_secs(&cfg), 900);
    }
    ...
    /// `Config::default()` plus the V3 single-agent bridge entries
    /// (`agents.default` + `risk_profiles.default`) that
    /// `run_gateway_webhook_agentic` resolves at turn time. Production
    /// configs get these from the V1→V3 migration; `Config::default()`
    /// deliberately has an empty agents map.
    ...
    /// Pairing disabled, no secret hash, NoopObserver. Caller provides
    /// provider+memory and the labels captured at "daemon startup".
    ...
    async fn webhook_without_model_returns_state_provider_and_model_in_body() {
        ...
        // Critical: MockModelProvider was actually invoked via state.model_provider,
    ...
    async fn webhook_rejects_unknown_provider_with_400() {
        ...
        let provider: Arc<dyn ModelProvider> = provider_impl.clone();
        let memory: Arc<dyn Memory> = Arc::new(MockMemory);
        let state = fresh_model_test_state(provider, memory, "mock", "test-model");
    ...
    fn make_test_state(provider: Arc<dyn ModelProvider>, memory: Arc<dyn Memory>) -> AppState {
        fresh_model_test_state(provider, memory, "mock", "test-model")
    }
