...
pub async fn run_gateway(
    ...
    // Captured as owned String here because `state.provider_name` outlives
    // the config snapshot (config can be hot-swapped via `model_routing_config
    // set_default` without rebuilding the provider Arc). See AppState diff in
    // docs/analysis/2026-05-20-v075-gateway-deep-dive.md (section "AppState diff").
