...
/// fork #7: look up the provider-specific environment variable for common
...
fn resolve_embedding_config(
    ...
    // Key resolution precedence (highest first), merging upstream v0.8.3 dotted
    // catalog-ref resolution with fork #7's provider-env lookup:
    //   1. per-route `api_key` override (routed branch) / `[memory].embedding_api_key` (base branch)
    //   2. the referenced provider profile's own key, when `model_provider` is a
    //      dotted `<type>.<alias>` catalog ref (resolved in `resolve_provider_ref`)
    //   3. broad provider-env lookup keyed by the embedding provider name
    //      (fork #7, e.g. OPENROUTER_API_KEY) — resolves our OpenRouter embedding
    //      key even when the chat provider carries none. Folded into the
    //      `inherited_api_key` slot so it sits below a resolved profile key but
    //      above the chat-seed key.
    //   4. the seed (chat) model provider's key, inherited via `api_key`
    // (1)/(2)/(3) let embeddings keep their own credential when the chat model
    // runs on a provider that carries no usable embedding key; (4) preserves the
    // prior behavior verbatim when none of the higher-precedence sources is set.
    // NOTE: for the common non-dotted base path (our Fly `embedding_provider =
    // "openrouter"`), `resolve_provider_ref` returns `explicit.or(inherited)` =
    // `configured.or(env).or(chat)`, exactly fork #7's original precedence.
    ...
    let configured_api_key = config
        ...
        .filter(|value| !value.is_empty())
        .map(str::to_string);
    // fork #7 (kept per R4 — carry #7 unless proven redundant): broad env lookup
    // for the embedding provider, folded into the inherited slot so it ranks
    // below a resolved catalog-profile key but above the chat-seed key. For the
    // non-dotted base path this yields `configured.or(env).or(chat)`, the
    // original fork #7 precedence; for a dotted ref it yields
    // `configured.or(profile_key).or(env).or(chat)`.
    let fallback = || {
        ...
        resolve_provider_ref(
            ...
            env_api_key.or_else(|| inherited_api_key.clone()),
            providers,
        )
    };
