...
impl SqliteMemory {
    ...
    ///
    /// A `session_id` filter still admits `core` rows (fork: Core-global
    /// recall) — cross-session "soul" facts must be recallable from sessions
    /// that did not write them, while every other row stays session-scoped.
