...
impl AuthService {
    ...
    pub async fn get_valid_gemini_access_token(
        ...
        // fork: wrap the refresh in a hard timeout (cross-daemon refresh lock
        // must never be held indefinitely); upstream added client_id/secret
        // pass-through — keep both.
        let refresh_outcome = tokio::time::timeout(
            Duration::from_secs(OAUTH_REFRESH_HTTP_TIMEOUT_SECS),
            refresh_gemini_access_token_with_retries(
                &self.client,
                client_id,
                client_secret,
                &refresh_token,
