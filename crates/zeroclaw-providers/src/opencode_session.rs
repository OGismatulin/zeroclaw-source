//! `x-opencode-session` and `User-Agent` — OpenCode relay identification.
//!
...
//! across the turns of a conversation. Since 2026-09 the Go endpoint rejects
//! header-less inference outright with HTTP 400 `MissingSessionID`. Upstream
//! also asks tools to identify themselves through `User-Agent`.
//!
//! Every OpenCode request — both wires, streaming and non-streaming — resolves
//! its session value through [`session_token`] and its UA through
//! [`OPENCODE_USER_AGENT`] so neither header can drift per code path.
//!
//! Ported from upstream PR zeroclaw-labs/zeroclaw#10604 (`51c1c04984`) with
//! the `User-Agent` addition. When that PR lands in a tagged upstream release,
//! keep the UA constant and drop the duplicated session injection.
//!
//! # Value derivation
//!
//! Upstream specifies no format, length, or lifetime for the session value;
//! its only normative statement is that a tool "includes the
//! `x-opencode-session` header so we can optimize prompt caching". This module
//! emits a 128-bit lowercase hex digest: opaque, fixed-length, unambiguously
//! safe as an HTTP header value, and non-reversible.
//!
...
//!
//! Session keys embed channel and user identifiers (`tg_user_<id>_s_<hex>`,
//! `whatsapp_123@g.us_alice`). Forwarding one verbatim would hand a
//! third-party relay a cross-service-linkable per-user identifier. Hashing
//! keeps the affinity behavior while sending nothing but an opaque token.
//!
...
//! key space. A per-install salt would cost the cross-restart affinity this is
//! for; it is deliberately out of scope here.
...
/// `User-Agent` sent to the OpenCode relay unless the operator pinned one.
pub const OPENCODE_USER_AGENT: &str = concat!("zeroclaw/", env!("CARGO_PKG_VERSION"));
...
///
/// Cron runs, CLI invocations and other calls that never enter
/// `TOOL_LOOP_SESSION_KEY` still have to carry a header, or the Go endpoint
/// would reject exactly those paths. One process-stable random token keeps
/// them pinned together without inventing a conversation identity for them.
...
mod tests {
    ...
    fn opencode_target_always_yields_a_token() {
        ...
        // unscoped cron/CLI calls are never header-less.
    ...
    #[test]
    fn user_agent_names_zeroclaw_and_version() {
        assert_eq!(
            OPENCODE_USER_AGENT,
            format!("zeroclaw/{}", env!("CARGO_PKG_VERSION"))
        );
        assert!(reqwest::header::HeaderValue::from_str(OPENCODE_USER_AGENT).is_ok());
    }
