...
// fork: `pub` retained — `zeroclaw-gateway::lib.rs` calls this directly.
...
// fork: `build_context` retained (upstream inlined/removed it). `zeroclaw-gateway`'s
// `run_gateway_webhook_agentic` calls this out-of-band — see usage note below.
...
///
/// `pub` retained from the fork — `zeroclaw-gateway`'s
/// `run_gateway_webhook_agentic` builds context out-of-band so it can
/// inject session-history without re-walking memory.
pub async fn build_context(
    ...
    match recall_result {
        Ok(mut entries) => {
            ...
            // fork: fully-qualified — upstream moved these imports to
            // `agent::memory_inject`; `build_context` keeps its own refs.
            zeroclaw_memory::decay::apply_time_decay(
                &mut entries,
                zeroclaw_memory::decay::DEFAULT_HALF_LIFE_DAYS,
            );
            ...
            if !relevant.is_empty() {
                ...
                for entry in &relevant {
                    ...
                    if !included {
                        context.push_str(zeroclaw_memory::MEMORY_CONTEXT_OPEN);
                        context.push('\n');
                        included = true;
                    }
                ...
                if included {
                    context.push_str(zeroclaw_memory::MEMORY_CONTEXT_CLOSE);
                    context.push_str("\n\n");
                }
            }
        }
    ...
    run_tool_call_loop(ToolLoop {
        exec: ResolvedAgentExecution::resolve(
            ...
            ResolvedIo {
                ...
                approval,
                multimodal_config,
                // fork: thread the caller's hooks (webhook passes state.hooks)
                // instead of upstream's hardcoded None.
                hooks,
                activated_tools,
        ...
        // fork: thread the caller's cancellation token (webhook latest-message-
        // wins) instead of upstream's hardcoded None.
        cancellation_token,
        on_delta: None,
...
pub async fn run(
    ...
    // #17 dropped: upstream `resolved_agent_for_turn` (PR #7583 "honor profile
    // tool iteration limits") internally calls `config.resolved_agent_config`
    // — the exact resolution our fork patch #17 added, so it carries the
    // runtime-profile `max_tool_iterations` (no longer capped at 10 on the
    // cron/CLI `run()` path). See project_cron_iteration_cap_10.
...
pub async fn process_message(
    ...
    use ::zeroclaw_log::Instrument;
    // #17 dropped: `resolved_agent_for_turn` (upstream #7583) resolves the
    // runtime-profile `max_tool_iterations` — equivalent to fork patch #17.
    let agent = resolved_agent_for_turn(&config, agent_alias)?;
    crate::agent::thinking::validate_thinking_config(&agent.resolved.thinking);
    let risk_profile = config
        .risk_profile_for_agent(agent_alias)
        ...
        zeroclaw_api::NATIVE_THINKING_OVERRIDE
            .scope(
                ...
                agent_turn(
                    ...
                    // fork: process_message path has no cancel surface / its own
                    // HookRunner (gateway injects one on the webhook path).
