...
pub(crate) fn resolve_display_text(
    ...
    // fork (2026-05-06 text-style tool-call display bug): both text-style and
    // native tool_calls drop ALL prose. The model can't have produced a
    // legitimate final answer in the same iteration as tool_calls — it has not
    // seen real tool results yet. Whatever prose is there (pre-call narration,
    // inter-call commentary, chain-of-thought, hallucinated <tool_result>,
    // premature final answer) must NOT reach the user; the next iteration
    // produces the correct answer from real tool output (CoT stays in
    // runtime-trace.jsonl). Upstream instead surfaces native prose alongside
    // tool calls — reverted here to keep tool calls invisible.
    if has_tool_calls {
        return String::new();
    }
