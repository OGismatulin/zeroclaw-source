...
impl DelegateTool {
    ...
    async fn execute_background(
        ...
        // Token registration is handled by upstream's process-global
        // `background_task_cancels()` map (inserted above, #8217) — fork #20's
        // per-instance `background_tasks` registry was removed as redundant.
    ...
    async fn handle_check_result(&self, args: &serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // fork DV-34312 poll-throttle (CARRIED onto the v0.8.3 seam): when the
        // task is still `Running`, long-poll up to `check_result_long_poll_secs`
        // (0 = off → immediate return) instead of returning at once. Each
        // `check_result` is one full LLM iteration of the caller; without this,
        // an orchestrator polling a slow sub-agent every ~5s drains its
        // `max_iterations` before launching downstream work (the coordinator
        // died mid-poll, one minute before the analyst finished, DV-34312). We
        // re-read the flat result each tick and return EARLY on any terminal /
        // reconciled-loss state; `Running` is returned only once the window
        // elapses.
        //
        // NB: upstream's newer `await` action (`handle_await_sessions`) is the
        // multi-task wait primitive and gives equivalent wait-without-burn
        // semantics; this long-poll is retained because the live jira
        // coordinator still polls via `check_result`. See task-6 report — drop
        // this (and the `check_result_long_poll_secs` schema field) once that
        // coordinator migrates to `await`.
        let deadline = tokio::time::Instant::now()
            + Duration::from_secs(self.delegate_config.check_result_long_poll_secs);
        const POLL_TICK: Duration = Duration::from_secs(5);
        let mut result = result;
        loop {
            // Capture the task's own failure text before `result` is consumed by
            // `background_result_view` (which overlays reconciled-loss).
            let file_error = result.error.clone();
            let (state, value) = Self::background_result_view(task_id, result).await?;

            // Fresh flat-result view FIRST: a task that just wrote an
            // authoritative terminal state (or a reconciled loss) wins over the
            // wait. Only `Completed` is success; other terminals are tool errors.
            if !state.is_pending() {
            ...
            // Re-read for the next tick; a vanished result file ends the wait.
            let Some(next) = self.read_background_result(task_id).await? else {
                return Ok(ToolResult {
                    success: false,
                    output: ToolOutput::default(),
                    error: Some(format!("No result found for task_id '{task_id}'")),
    ...
    async fn handle_list_results(&self) -> anyhow::Result<ToolResult> {
        ...
        while let Some(entry) = entries.next_entry().await? {
                ...
                && let Ok(result) = serde_json::from_str::<BackgroundDelegateResult>(&content)
            {
                ...
                // fork(#20): build a mutable row so the heartbeat sidecar overlay
                // below can add `updated_at` before pushing.
                let mut row = json!({
    ...
    async fn handle_cancel_task(&self, args: &serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // Real cancellation via upstream's process-global token map (#8217):
        // signalling the registered token cascades into the task's
        // `tokio::select!`, which settles it as Cancelled and actually stops the
        // sub-agent burning provider/VM. Unlike fork #20's per-instance
        // registry, the global map can cancel a task spawned by an earlier turn
        // / before a daemon respawn. Falls back to file-marking when the task
        // already settled (token absent).
